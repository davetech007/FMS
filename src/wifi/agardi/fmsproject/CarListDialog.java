...
public class CarListDialog extends Dialog<CarFX> {
	...
	private TableView<CarFX> carsTableView() {
		...
		carsTableView.getColumns().addAll(numberCol, categorieCol, markeCol, modellCol, licPlateCol, vinNumCol,
											fuelTypeCol);
		carsTableView.getSortOrder().addAll(categorieCol, licPlateCol);
		carsTableView.setPlaceholder(new Label("No cars available!"));

		numberCol.setCellValueFactory(
				column -> new ReadOnlyObjectWrapper<Number>(carsTableView.getItems().indexOf(column.getValue()) + 1));
